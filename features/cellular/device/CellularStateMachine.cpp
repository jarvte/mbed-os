...
nsapi_error_t CellularStateMachine::continue_to_state(CellularState state)
{
    _retry_count = 0;
    if (state < _state) {
        _state = state;
...
void CellularStateMachine::set_callback(mbed::Callback<bool(int, int, int)> status_callback)
{
